// ─────────────────────────────────────────────────────────────────────────────
// components/Footer.tsx — Site footer with copyright, legal links and social
// ─────────────────────────────────────────────────────────────────────────────

import { BORDER, MUTED, FONT_BODY } from '../constants'

const YEAR = new Date().getFullYear()

// Social media icons as inline SVGs (brand icons aren't in lucide-react)
const SocialIcon = ({ path, viewBox = '0 0 24 24' }: { path: string; viewBox?: string }) => (
  <svg width={20} height={20} viewBox={viewBox} fill="currentColor" aria-hidden="true">
    <path d={path} />
  </svg>
)
...
const SOCIAL = [
  ...
  {
    label: 'YouTube',
    href:  'https://youtube.com/@kxptechnologies',
...
export default function Footer() {
  return (
    ...
    >
      ...
      {/* Copyright */}
      <p style={{ textAlign: 'center', fontSize: 12, color: MUTED, margin: 0, lineHeight: 1.6 }}>
        © {YEAR} Kiran Parmar · KXP Technologies. All rights reserved.
      </p>
    </footer>
  )
}
