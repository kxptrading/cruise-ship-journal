...
export function useVoyageData({ session, showToast }: Options): UseVoyageDataReturn {
  ...
  const update = useCallback((key: keyof VoyageData, val: VoyageData[keyof VoyageData]) => {
    ...
    if (voyageId) persistLocal(key, voyageId, val)
    ...
    if (key === 'foodLogs' && voyageId) {
      ...
      foodLogsTimer.current = window.setTimeout(async () => {
        ...
        if (rows.length > 0) {
          syncCheck(await supabase.from('food_logs').upsert(toDbFoodLogs(voyageId, rows), { onConflict: 'id' }), 'foodLogs', val)
          const ids = rows.map(r => r.id)
          syncCheck(await supabase.from('food_logs').delete().eq('voyage_id', voyageId).not('id', 'in', `(${ids.join(',')})`))
        } else {
          syncCheck(await supabase.from('food_logs').delete().eq('voyage_id', voyageId), 'foodLogs', val)
        }
      }, 800)
    }
    if (key === 'diningLog' && voyageId) {
      ...
      diningLogTimer.current = window.setTimeout(async () => {
        ...
        if (rows.length > 0) {
          syncCheck(await supabase.from('dining_log').upsert(toDbDiningLog(voyageId, rows), { onConflict: 'id' }), 'diningLog', val)
          const ids = rows.map(r => r.id)
          syncCheck(await supabase.from('dining_log').delete().eq('voyage_id', voyageId).not('id', 'in', `(${ids.join(',')})`))
        } else {
          syncCheck(await supabase.from('dining_log').delete().eq('voyage_id', voyageId), 'diningLog', val)
        }
      }, 800)
    }
    if (key === 'entertainmentLog' && voyageId) {
      ...
      entertainmentTimer.current = window.setTimeout(async () => {
        ...
        if (rows.length > 0) {
          syncCheck(await supabase.from('entertainment_log').upsert(toDbEntertainmentLog(voyageId, rows), { onConflict: 'id' }), 'entertainmentLog', val)
          const ids = rows.map(r => r.id)
          syncCheck(await supabase.from('entertainment_log').delete().eq('voyage_id', voyageId).not('id', 'in', `(${ids.join(',')})`))
        } else {
          syncCheck(await supabase.from('entertainment_log').delete().eq('voyage_id', voyageId), 'entertainmentLog', val)
        }
      }, 800)
    }
    if (key === 'shopping' && voyageId) {
      ...
      shoppingTimer.current = window.setTimeout(async () => {
        ...
        if (s.items?.length > 0) {
          syncCheck(await supabase.from('shopping_items').upsert(toDbShoppingItems(voyageId, s.items), { onConflict: 'id' }), 'shopping', val)
          const ids = s.items.map(i => i.id)
          syncCheck(await supabase.from('shopping_items').delete().eq('voyage_id', voyageId).not('id', 'in', `(${ids.join(',')})`))
        } else {
          syncCheck(await supabase.from('shopping_items').delete().eq('voyage_id', voyageId), 'shopping', val)
        }
      }, 800)
    }
    if (key === 'packing' && voyageId) {
      ...
      packingTimer.current = window.setTimeout(async () => {
        ...
        syncCheck(await supabase.from('packing_items').delete().eq('voyage_id', voyageId), 'packing', val)
    ...
    if (key === 'notes' && voyageId) {
      ...
      notesTimer.current = window.setTimeout(async () => {
        ...
        if (n.length > 0) {
          syncCheck(await supabase.from('notes').upsert(toDbNotes(voyageId, n), { onConflict: 'id' }), 'notes', val)
          const ids = n.map(note => note.id)
          syncCheck(await supabase.from('notes').delete().eq('voyage_id', voyageId).not('id', 'in', `(${ids.join(',')})`))
        } else {
          syncCheck(await supabase.from('notes').delete().eq('voyage_id', voyageId), 'notes', val)
        }
      }, 800)
    }
    if (key === 'budget' && voyageId && budgetIdRef.current) {
      ...
      budgetTimer.current = window.setTimeout(async () => {
        ...
        const opts = { budgetId: bid ?? undefined }
