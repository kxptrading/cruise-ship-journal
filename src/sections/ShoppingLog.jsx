    ...
    <div>
      ...
      <div style={cs}>
            ...
            <thead>
              <tr style={{ background: NAVY }}>
                {['Item', 'Port / Location', 'Cost (£)', ''].map(h => (
                  <th key={h} style={{ padding: w < BP.mobile ? '9px 8px' : '10px 12px', color: WHITE, fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textAlign: 'left' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? WHITE : LIGHT, borderBottom: `1px solid ${BORDER}` }}>
                  ...
                  <td style={{ padding: w < BP.mobile ? '6px 8px' : '6px 10px' }}>
                    <input value={item.port || ''} onChange={e => set(i, 'port', e.target.value)} placeholder="Port"
                  ...
                  <td style={{ padding: w < BP.mobile ? '6px 6px' : '6px 10px' }}>
                    <input type="number" value={item.cost || ''} onChange={e => set(i, 'cost', e.target.value)} placeholder="0.00"
                      style={{ border: 'none', background: 'transparent', fontSize: 13, color: TEXT, width: w < BP.mobile ? 60 : 90 }} />
                  </td>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginTop: 16, paddingTop: 16, borderTop: `1px solid ${BORDER}` }}>
          ...
          <div style={{ fontSize: w < BP.mobile ? 15 : 18, fontWeight: 700, color: NAVY, fontFamily: 'Georgia,serif' }}>TOTAL: £{total.toFixed(2)}</div>
        </div>
      </div>
    </div>
  )
